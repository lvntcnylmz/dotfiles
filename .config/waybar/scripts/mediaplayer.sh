...
spotify_title=$(playerctl -p spotify metadata --format "{{ emoji(status) }} {{ artist }} - {{ title }} - {{ duration(position) }} | {{ duration(mpris:length) }}")
firefox_title=$(playerctl metadata --format "{{ emoji(status) }} {{ artist }} - {{ title }}")
